...
import { useChat, useConversations } from '@/features/chat/hooks/useChat';
...
import { PingsModal } from '../pings/PingsModal';
import { cn } from '@/shared/lib/utils';
import { useTypingIndicator, useSocketStore } from '@/socket/hooks/useSocket';
import { EVENTS } from '@/socket/events/events';
import { formatMessageDay, formatMessageTime } from '@/shared/utils/dateUtils';
import { useMessageGrouping } from '../../hooks/useMessageGrouping';
import { useContacts } from '../../hooks/useContacts';
import { usePingStatus } from '../../hooks/usePingStatus';
import { MessageContextMenu } from '@/shared/components/ui/MessageContextMenu';
...
export default function ChatLayout() {
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent | React.TouchEvent, message: ChatMessage) => {
    e.preventDefault();
    let x, y;
    if ('touches' in e) {
    ...
    setContextMenu({ isOpen: true, message, position: { x, y } });
  }, []);

  const handleEdit = () => console.log('Edit', contextMenu.message?.id);
  const handleDelete = () => console.log('Delete', contextMenu.message?.id);
  const handleShowDetails = () => console.log('Details', contextMenu.message?.id);

  const { isTyping, typingUsers } = useTypingIndicator(selectedUser || undefined);

  const contacts = useContacts(conversations, incoming, outgoing);
  const { selectedUserSummary, pingStatus, isPingAccepted, incomingPing } = usePingStatus(selectedUser, conversations, incoming, outgoing);
  ...
  const messageGroups = useMessageGrouping(
    useMemo(() => allMessages.map((doc: any) => parseMessage(doc, userId || '')), [allMessages, userId])
  );
            ...
            {isPingAccepted ? (
              <div className="flex-1 overflow-y-auto flex flex-col-reverse p-4 space-y-reverse space-y-6 scroll-smooth overscroll-contain">
               ...
               {messageGroups.map((group, groupIndex) => {
                   const isMe = group.isOwn;
                   const nextGroup = messageGroups[groupIndex + 1];
                   const showDateHeader = !nextGroup || 
                     formatMessageDay(group.date) !== formatMessageDay(nextGroup.date);

                   return (
                     <div key={group.id} className="flex flex-col w-full min-w-0 mb-4">
                       <div
                         className={cn(
                           "flex flex-col max-w-[85%] md:max-w-[70%] relative group min-w-0",
                       ...
                       >
                         {group.messages.map((message, index) => {
                           ...
                           return (
                             <div key={message.id} className={cn("flex flex-col w-full relative group", isLastInGroup ? "mb-1" : "mb-[2px]")}
                         ...
                         <div className={cn(
                           ...
                           <span>
                             {formatMessageTime(new Date(group.messages[group.messages.length - 1].createdAt))}
                           </span>
                           {isMe && (
                             <span 
                               className={cn(
                                 "flex items-center",
                                 group.messages[group.messages.length - 1].status === 'read' ? "text-blue-500" : ""
                               )}
                             >
