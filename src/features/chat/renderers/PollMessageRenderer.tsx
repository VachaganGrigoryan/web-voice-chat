...
export const PollMessageRenderer: React.FC<PollMessageRendererProps> = ({
  ...
  const { poll, isLoading, isError, vote, retract, close, isVoting, isClosing } = usePoll(
    message.pollId,
    { enabled: Boolean(message.pollId) }
  );
  ...
  const shell = (children: React.ReactNode) => (
    ...
    >
      <MessageContent className="min-w-[16rem] max-w-[22rem]">{children}</MessageContent>
      {bubbleFooter}
    </MessageBubble>
  );
    ...
    return shell(
      <>
        ...
        <div className="flex items-center gap-2 text-sm text-current/70">
          ...
          <span className="truncate">{message.question}</span>
        </div>
      </>
    );
  }
  ...
  if (isError || !poll) {
    return shell(
      <>
        {header}
        <div className="text-sm font-semibold">{message.question}</div>
