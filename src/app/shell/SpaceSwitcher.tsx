...
export function SpaceSwitcher({
    ...
    <>
      <div className="max-h-60 overflow-y-auto">
        <button
          ...
          className={cn(
            "mb-0.5 flex w-full items-center justify-between rounded-lg p-2 text-xs font-medium transition-colors",
        ...
        >
          <div className="flex min-w-0 items-center gap-2">
            <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border bg-muted text-muted-foreground">
              <Globe className="h-4 w-4" />
            </div>
            <div className="min-w-0 text-left">
              <div className="truncate">Global</div>
        ...
        {spaces.map((space) => {
          const isVogi = space.is_default || space.slug === 'vogi';
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                <div className={cn(
                  "flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border overflow-hidden p-0.5",
                  isVogi
                    ? "bg-slate-900 border-slate-700/60 shadow-sm"
                    : "bg-muted text-muted-foreground font-bold uppercase text-[10px]"
                )}>
                  {isVogi ? <LogoSymbol size="sm" /> : space.name[0]}
                </div>
                <span className="truncate">
                  {space.name} {isVogi ? '(Public)' : ''}
                </span>
              </div>
  ...
  const isGlobalActive = selectedSpaceId === null;
  const isVogiActive =
    selectedSpace !== null && (selectedSpace.is_default || selectedSpace.slug === 'vogi');
  const spaceInitial = selectedSpace ? selectedSpace.name[0].toUpperCase() : null;
  const triggerTitle = selectedSpace?.name ?? 'Global';
  const triggerIcon = isVogiActive
    ? <LogoSymbol size="sm" />
    : isGlobalActive
      ? <Globe className="h-4 w-4" />
      : spaceInitial;
    ...
    return (
      <div className="relative">
        <button
          ...
          className={cn(
            "flex shrink-0 items-center justify-center text-white font-semibold transition-all overflow-hidden p-1 border",
            "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40",
            isRail ? "h-11 w-11 rounded-2xl text-sm" : "h-9 w-9 rounded-xl text-xs",
            isVogiActive
              ? "bg-slate-900 border-slate-700/60 shadow-md shadow-slate-900/20"
              : isGlobalActive
                ? "bg-muted text-foreground border-border shadow-md shadow-foreground/5"
              : "bg-gradient-to-tr from-indigo-500 to-purple-500 border-indigo-400/30 shadow-md shadow-indigo-500/10",
    ...
    <div className="relative w-full px-4 pt-3">
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0">
          <div className={cn(
            "flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-white font-semibold text-xs border overflow-hidden p-0.5",
            isVogiActive
              ? "bg-slate-900 border-slate-700/60 shadow-sm"
              : isGlobalActive
                ? "bg-muted text-foreground border-border shadow-sm"
              : "bg-gradient-to-tr from-indigo-500 to-purple-500 border-indigo-400/30 shadow-sm"
          )}>
            {triggerIcon}
          </div>
